...
- Recipes (v1 names only): summarize, extract, translate keeping format, redact, convert to Markdown, combine several items into one document. Images also get on-device “Get Text” (Vision OCR → `ocr.md`), which does not need a CLI agent.
