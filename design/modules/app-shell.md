...
- `NSStatusItem` 靠右。
- 面板默认约 800pt 宽、640pt 高，贴图标下方。工作或结果收起时变窄；只留输入时约 260–280pt。顶栏：字标左、搜索中、芯片和栏 / 设置 / 窗口按钮右。搜索下拉加入架子。左「输入」与右「结果」列头对齐；中间动作 / 终端 / 预览是分开的 Tag。头上关闭左边是最小化，都是藏面板。
- 人离开后面板闲时：变淡并降到普通窗口层级，让开底下的 App。点还露着的面板、把文件拖进面板区域、图标或开合快捷键会醒；闲时点图标是唤醒不是关。从面板拖出时不闲时。诊断启动不闲时。
- 第一次打开若抓页权限未齐，就绪卡盖住三栏（头仍在）。设置左右分栏：使用准备、快捷键、能做什么、本机、外观。外观可关工作栏、结果栏、拖放轮盘。准备好抓页不要求每一家已装浏览器都授权。
