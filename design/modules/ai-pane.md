...
| Tab | 何时出现内容 |
|-----|----------------|
| 动作 | 默认。六个 Recipe 纵列：左纸面按钮，右一句说明。芯片对应 CLI 没有可收口入口时 Recipe 禁用；无 TUI 时发送禁用，留安装引导。旧数据里选中已完成、或失败但已有 output：不画 Recipe，指向拿走；后者可「再跑一次」。新 Job 跑完输入仍是 idle，动作区仍是 Recipe。已进终端的条目同样留着这列动作。 |
