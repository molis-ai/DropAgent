...
发现本机 CLI、给出隔离档文案、产出「怎么跑」的执行配置。Recipe 和发送都跟芯片。预置 TUI：Grok / Claude / Gemini / OpenCode / Cursor CLI / Codex。预置纯 CLI：llm / aichat / sgpt。设置里可添加自定义 Runtime。没有可收口 Job 入口的引擎，`recipePresence` 为 `.none`。
...
- 探测：`PATH` 与 `~/.grok/bin`、`~/.local/bin`、`~/.opencode/bin`、`~/.cursor/bin` 等，再设置里用户指定 / 自定义。文件名 `agent` 用 `--help` 区分 Grok Build 与 Cursor CLI；不要把 `~/.grok/bin/agent` 认成 Cursor。
- Recipe 跟 `tuiPresence` 同一 Runtime。优先官方非交互入口（`codex exec`、`grok --prompt-file` / `--single`、`claude -p/--print`、`opencode run`、Cursor `-p/--print`、Gemini / llm 按其 `--help`），以本机 `--help` 为准。cwd = `work/`。不传 `--auto` / `--yolo` / `--dangerously-skip-permissions`。
