...
点「复制」把 CLIP 正文交给备忘录。架子上贴进一条网址，会抓成网站条；复制交出文件夹（链接 / 正文 / 截图）。纯 URL 条仍由拖到 AI 区产生，内核 Check 覆盖其复制不落 `link.txt`。
...
3. `--e2e`：粘贴 https 后 kind 是 WEB；复制后有文件 URL（网站文件夹）。
