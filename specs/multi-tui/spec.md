...
面板里的「发送 / 终端」现在写死 Codex。本机已经有 Grok Build（`grok`）和 Claude Code（`claude`）。人要在设置里选终端用哪个；Grok 和其他已装 TUI 都要能进内嵌 PTY。Recipe 仍只保证 Codex `exec` 沙箱。
...
- `AgentSettings.tuiEngine`：`auto | grok | claude | gemini | codex`，可另存每个引擎的可执行文件路径。旧 `executableOverride` 仍当 Codex 路径。
- `auto` 顺序：Grok → Claude → Gemini → Codex（只在已装里选）。
- 明确选了未装的引擎：终端 = 没有，不偷偷换成别的。
- TUI 启动按引擎拼参数和环境；隔离 home，不把用户全局 MCP / Hooks / 原件路径送进会话。
- 头上芯片和右键菜单可选终端；换引擎后下一次发送开新会话。
- Recipe / `Job.start` 仍要求 Codex；没有 Codex 时 Recipe 禁用，有 TUI 仍可发送。

## 非目标

- 用 Grok/Claude/Gemini 跑六个 Recipe。
...
1. 装了 Grok：打开面板头上是 Grok，发送进 Grok TUI；总结仍用 Codex。
2. 设置里改成 Codex：发送进 Codex；Grok 仍显示为已装可选项。
3. 只装 Claude：auto 用 Claude；Recipe 禁用并写明需要 Codex。
4. 选 Gemini 但没装：发送禁用，芯片写未发现 Gemini。
...
- `DropAgentJob`：继续 `discover()` 且必须是 Codex。
- `macos/App`：设置菜单、芯片、文案。不在 View 里搜 PATH。
- 更新 `design/modules/{agent,tui,ai-pane,app-shell}.md` 与 `03-tech-architecture.md` §7。

## 验收标准

1. 同时存在 Grok 与 Codex、`tuiEngine=auto`：`tuiPresence` 是 Grok，`recipePresence` 是 Codex。
2. `tuiEngine=codex`：终端是 Codex，即使 Grok 在。
...
5. 无 Codex、有 Grok：Recipe 禁用，发送可用。
