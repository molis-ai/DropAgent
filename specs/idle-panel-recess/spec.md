...
- **闲时**（可见、不是 key、鼠标不在面板上、不是从面板往外拖、不是诊断启动）：约 0.7 秒后透明度 0.4，层级降到 `.normal`。
- **醒**：点还露着的面板（第一下也算正常点击）；外部拖进面板矩形外扩 12pt；图标或开合快捷键（闲时是唤醒，不是关）；`showPanel`。
- 拖去 Mail / 桌面：继续躲着。从面板拖出：中途不淡、不沉。Esc / 叉 / 最小化仍是关掉。
...
2. `shouldRecess`：失焦且鼠标在外才闲时；key / 鼠标在内 / 往外拖 / 诊断不闲时。
3. 拖点在面板框外扩 12pt 内算命中，更远不算。
4. `applyRecess` 后 alpha 0.4、level normal；`applyActive` 后 alpha 1、level statusBar。
