...
enum WheelLayout {
    static func slices(hasAgent: Bool, hasRecipe: Bool) -> [WheelSlice] {
        [
            WheelSlice(action: .shelf, title: Copy.t("加入架子", "Shelf"), symbol: "tray.and.arrow.down", enabled: true),
            WheelSlice(action: .send, title: Copy.t("发给", "Send"), symbol: "arrow.right", enabled: hasAgent),
