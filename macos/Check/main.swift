...
private func browserFront() throws {
    ...
    expect(AccessibilityPage.resolveTrusted(processFlag: true, apiEnabled: false), "process flag is enough")
    expect(AccessibilityPage.resolveTrusted(processFlag: false, apiEnabled: true), "live AX read is enough when the process flag lags")
    expect(AccessibilityPage.resolveTrusted(processFlag: false, apiEnabled: false) == false, "do not fake accessibility trust")
    let axStart = Date()
